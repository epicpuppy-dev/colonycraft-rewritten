...
export class OverlayInventory extends Screen {
    ...
    public render(game: typeof ColonyCraft, ctx: OffscreenCanvasRenderingContext2D): void {
        ...
        ctx.fillRect(Math.floor(this.width / 8 + 48), Math.floor(7 * this.height / 8 - 18), Math.min(Math.floor((3 * this.width / 4 - 56) * (game.inventory.storageUsed / game.inventory.storageCapacity)), (3 * this.width / 4 - 56)), 10);
        ...
        */
        ...
        let maxItems = 0;
        for (let i = 0; i < Object.keys(game.inventory.items).length; i++) {
            if (game.inventory.items[Object.keys(game.inventory.items)[i]].amount > maxItems) maxItems = game.inventory.items[Object.keys(game.inventory.items)[i]].amount;
        }
        //get width of maxItems
        const maxwidth = game.draw.textWidth(`${maxItems.toLocaleString()}`, 14);

        //loop through all inventory categories
        for (let i = 0; i < Object.keys(game.inventory.categories).length; i++) {
            if (currentColumn > 0) {
                currentColumn = 0;
                currentRow++;
            }
            ...
            let hasItems = false;
            for (let j = 0; j < game.inventory.categories[Object.keys(game.inventory.categories)[i]].items.length; j++) {
                if (game.inventory.categories[Object.keys(game.inventory.categories)[i]].items[j].amount > 0) {
                    hasItems = true;
                    break;
                }
            }
