...
export class Overlay2Load extends Screen {
    ...
    constructor (game: ColonyCraft, width: number, height: number) {
        ...
        this.scrollBar = new ScrollBar(game, Math.floor(3 * this.width / 4 - 24), Math.floor(this.height / 8 + 50), 16, Math.floor(3 * this.height / 4 - 150), "v", 0, 5, 5, 48, game => game.currentScreens.includes("load"));
    ...
    public render(game: ColonyCraft, ctx: OffscreenCanvasRenderingContext2D): void {
        ...
        let color = save ? "#ffffff" : "#777777";
        game.draw.textCenter("Load", Math.floor(5 * this.width / 8 - 2.5), Math.floor(7 * this.height / 8 - 32), 14, color);
